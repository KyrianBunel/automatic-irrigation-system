...
def manu():
    ...
    mode = 'MANU'
    mqtt_client.publish(MQTT_BASE_TOPIC + "mode", 'MANU', retain=True)
    print(f"mode = {mode}")
    return redirect(url_for('accueil'), code=302)
...
def auto():
    global mode  # Accéder aux variables globales
    mode = 'AUTO'
    mqtt_client.publish(MQTT_BASE_TOPIC + "mode", 'AUTO', retain=True)
    print(f"mode = {mode}")
    return redirect(url_for('accueil'), code=302)
...
def prog():
    global mode  # Accéder aux variables globales
    mode = 'PROG'
    mqtt_client.publish(MQTT_BASE_TOPIC + "mode", 'PROG', retain=True)
    print(f"mode = {mode}")
    return redirect(url_for('accueil'), code=302)
...
def Vanne1():
    ...
    if vanne1 == 0:
        vanne1 = 1
        mqtt_client.publish(MQTT_BASE_TOPIC + 'vanne1', '1', retain=True)
        zone1_class = "rectangleON"
        zone1_text = "ON"
    else:
        vanne1 = 0
        mqtt_client.publish(MQTT_BASE_TOPIC + 'vanne1', '0', retain=True)
...
def Vanne2():
    ...
    if vanne2 == 0:
        vanne2 = 1
        mqtt_client.publish(MQTT_BASE_TOPIC + 'vanne2', '1', retain=True)
        zone2_class = "rectangleON"
        zone2_text = "ON"
    else:
        vanne2 = 0
        mqtt_client.publish(MQTT_BASE_TOPIC + 'vanne2', '0', retain=True)
...
def Vanne3():
    ...
    if vanne3 == 0:
        vanne3 = 1
        mqtt_client.publish(MQTT_BASE_TOPIC + 'vanne3', '1', retain=True)
        zone3_class = "rectangleON"
        zone3_text = "ON"
    else:
        vanne3 = 0
        mqtt_client.publish(MQTT_BASE_TOPIC + 'vanne3', '0', retain=True)
...
def submit():
    ...
    # Récupération et conversion des dates (remplace les tirets - par des slashs /)
    raw_date_debut = request.form.get("dateDebut", "")
    raw_date_fin = request.form.get("dateFin", "")
    StrDateDebut = raw_date_debut.replace("-", "/") if raw_date_debut else "0000/00/00"
    StrDateFin = raw_date_fin.replace("-", "/") if raw_date_fin else "0000/00/00"

    # Récupération des autres données du formulaire
    StrDuree = request.form.get("duree", "0")
    StrRecurrence = request.form.get("recurrence", "0")
    StrZone1 = request.form.get("zone1")
    StrZone2 = request.form.get("zone2")
    StrZone3 = request.form.get("zone3")
    StrDebut1 = request.form.get("heureDebut1", "00:00")
    StrDebut2 = request.form.get("heureDebut2", "00:00")
    StrDebut3 = request.form.get("heureDebut3", "00:00")
    ...
    params = {
        ...
        "StrZone1": "on" if StrZone1 else "off",
        "StrZone2": "on" if StrZone2 else "off",
        "StrZone3": "on" if StrZone3 else "off",
        "StrDebut1": StrDebut1,
        "StrDebut2": StrDebut2,
        "StrDebut3": StrDebut3,
    }
    ...
    return redirect(url_for('prog'), code=302)
...
if __name__ == '__main__':
    ...
    for key, val in params.items():
        mqtt_client.publish(MQTT_BASE_TOPIC + key, val, retain=True)
    """
